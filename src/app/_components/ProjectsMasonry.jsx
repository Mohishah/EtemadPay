...
const ProjectsMasonry = ({ project, categories }) => {
    // Isotope
    ...
    const handleFilterKeyChange = (key, e) => {
        e.preventDefault();
        setFilterKey(key);
        const filterLinks = document.querySelectorAll(".mil-filter p");
      ...
      <>
        ...
        <div className="mil-filter mil-up mil-mb-90">
            <div className="mil-filter-links">
                <p data-filter="*" className="mil-current" onClick={ (e) => handleFilterKeyChange("*", e)}>همه</p>
                {categories.map((item, key) => (
                <p data-filter={`${item.slug}`} key={`projects-filter-item-${key}`} onClick={(e) => handleFilterKeyChange(item.slug, e)}>{item.name}</p>
                ))}
            </div>
        </div>
        {/* filter end */}
                ...
                <div className={`mil-grid-item ${item.category_slug}`} key={`projects-item-${key}`}>
                    ...
                    <Link href={`/projects/${item.id}`} className={item.orientation == "vertical" ? "mil-portfolio-item-2 mil-long-item mil-mb-30" : "mil-portfolio-item-2 mil-square-item mil-mb-30"}>
                        ...
                        <div className="mil-project-descr">
                            ...
                            <div className="mil-link mil-upper">مشاهده بیشتر <div className="mil-arrow mil-light"><img src="/img/icons/1.svg" alt="arrow" /></div></div>
                        </div>

                        <div className="mil-category">{item.category_name}</div>
                    </Link>
